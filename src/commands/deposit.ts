...
import { info, success, warn, spinner, assertApiOk, wrapAction, requireTransactionConfirmation } from '../utils.js';
...
const perpsCmd = new Command('perps')
  ...
  .action(wrapAction(async (opts: { amount?: string; address?: boolean; yes?: boolean }) => {
    const creds = requireAuth();
    await perpsDepositFlow(creds.accessToken, opts);
  }));

async function perpsDepositFlow(token: string, opts?: { amount?: string; address?: boolean; yes?: boolean }): Promise<void> {
